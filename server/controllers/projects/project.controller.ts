  ...
  try {
    ...
    if (date && typeof date === "string") {
      ...
      if (date.match(/^\d{4}-\d{2}$/)) {
        ...
        baseQuery +=
          " AND (DATE_FORMAT(p.due_date, '%Y-%m') = ? OR DATE_FORMAT(m.due_date, '%Y-%m') = ?)";
        params.push(date, date);
      } else if (date.match(/^\d{4}-\d{2}-\d{2}$/)) {
        // Filter by exact date
        baseQuery += " AND (DATE(p.due_date) = ? OR DATE(m.due_date) = ?)";
        params.push(date, date);
      } else {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      return res
        .status(200)
        .json({ success: true, projects: [], message: "No projects found." });
    }
...
export const updateProjectController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user_id || !project_id) {
      return res
        .status(400)
        .json({ success: false, error: "user & project_id required" });
    }
    ...
    if (
      Array.isArray(removed_milestone_ids) &&
      removed_milestone_ids.length > 0
    ) {
      const placeholders = removed_milestone_ids.map(() => "?").join(",");
      await conn.execute(
        `DELETE FROM milestones WHERE milestone_id IN (${placeholders})`,
        removed_milestone_ids,
      );
    }
    ...
    const counts: { total: number; completed: number } =
      Array.isArray(countsRows) && countsRows.length > 0
    ...
    insertLog(
      ...
      username2,
      10,
      `Project Update: The Project '${project_name}' was updated  by ID: ${user_id}`,
    );
...
export const createProjectController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    insertLog(
      ...
      username2,
      10,
      `New Project '${project_name}' was created  by user ID: ${user_id}`,
    );
    await pushNotifications(
      "project",
      user_id,
      ` New Project '${project_name}' was created`,
      row.project_name,
      user_role,
      "allUsers",
      req,
...
export const getProjectNamesController = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const getMilestoneNames = async (req: Request, res: Response) => {
  try {
    ...
    const [rows] = await pool.execute(
      `select milestone_name,milestone_id from milestones where project_id =? `,
      [project_id],
    );
    if (!Array.isArray(rows) || rows.length === 0) {
      return res.status(201).json({
...
export const getSimpleProjectsController = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const deleteProjectController = async (req: Request, res: Response) => {
  try {
    ...
    insertLog(
      ...
      username2,
      10,
      `Project Delete :'Project ID : ${project_id}' was deleted  by user ID: ${user_id}`,
    );
